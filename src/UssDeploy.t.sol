...
import "./UssDeploy.t.base.sol";

contract UssDeployTest is UssDeployTestBase {
    ...
    function testFrobDrawDai() public {
        deploy();
        assertEq(usdai.balanceOf(address(this)), 0);
        weth.mint(1 ether);
        weth.approve(address(ethJoin), uint(-1));
        ethJoin.join(address(this), 1 ether);
        ...
        assertEq(vat.usdai(address(this)), mul(RAY, 60 ether));

        vat.hope(address(usdaiJoin));
        usdaiJoin.exit(address(this), 60 ether);
        assertEq(usdai.balanceOf(address(this)), 60 ether);
        assertEq(vat.usdai(address(this)), 0);
    }
    ...
    function testFrobDrawDaiGem() public {
        ...
        vat.hope(address(usdaiJoin));
        usdaiJoin.exit(address(this), 20 ether);
        assertEq(usdai.balanceOf(address(this)), 20 ether);
    }
    ...
    function testFrobDrawDaiLimit() public {
        ...
        weth.approve(address(ethJoin), uint(-1));
        ethJoin.join(address(this), 1 ether);
        vat.frob("ETH", address(this), address(this), address(this), 0.5 ether, 100 ether); // 0.5 * 300 / 1.5 = 100 USDAI max
    }
    ...
    function testFrobDrawDaiGemLimit() public {
        ...
        col.approve(address(colJoin), 1 ether);
        colJoin.join(address(this), 1 ether);
        vat.frob("COL", address(this), address(this), address(this), 0.5 ether, 20.454545454545454545 ether); // 0.5 * 45 / 1.1 = 20.454545454545454545 USDAI max
    }
    ...
    function testFrobPaybackDai() public {
        ...
        usdaiJoin.exit(address(this), 60 ether);
        assertEq(usdai.balanceOf(address(this)), 60 ether);
        usdai.approve(address(usdaiJoin), uint(-1));
        usdaiJoin.join(address(this), 60 ether);
        assertEq(usdai.balanceOf(address(this)), 0);
    ...
    function testFailBite() public {
        ...
        weth.approve(address(ethJoin), uint(-1));
        ethJoin.join(address(this), 1 ether);
        vat.frob("ETH", address(this), address(this), address(this), 0.5 ether, 100 ether); // Maximun USDAI
    ...
    function testBite() public {
        deploy();
        this.file(address(cat), "ETH", "dunk", rad(200 ether)); // 200 USDAI max per batch
        this.file(address(cat), "box", rad(1000 ether)); // 1000 USDAI max on auction
        this.file(address(cat), "ETH", "chop", WAD);
        weth.mint(1 ether);
        weth.approve(address(ethJoin), uint(-1));
        ethJoin.join(address(this), 1 ether);
        vat.frob("ETH", address(this), address(this), address(this), 1 ether, 200 ether); // Maximun USDAI generated
    ...
    function testBitePartial() public {
        ...
        vat.frob("ETH", address(this), address(this), address(this), 10 ether, 2000 ether); // Maximun USDAI generated
    ...
    function testClip() public {
        ...
        this.file(address(dog), "Hole", rad(1000 ether)); // 1000 USDAI max on auction
        this.file(address(dog), "COL2", "hole", rad(1000 ether)); // 1000 USDAI max on auction
        this.file(address(dog), "COL2", "chop", WAD);
        col2.mint(1 ether);
        col2.approve(address(col2Join), uint(-1));
        col2Join.join(address(this), 1 ether);
        vat.frob("COL2", address(this), address(this), address(this), 1 ether, 20 ether); // Maximun USDAI generated
    ...
    function testFlop() public {
        ...
        assertEq(vat.usdai(address(vow)), 0);
    ...
    function _flap() internal returns (uint batchId) {
        ...
        assertEq(vat.usdai(address(vow)), 0);
        jug.drip("ETH");
        assertEq(vat.usdai(address(vow)), rad(10 * 0.05 ether));
        ...
        assertEq(usdai.balanceOf(address(user1)), 0);
    ...
    function testFlap() public {
        ...
        user1.doHope(address(vat), address(usdaiJoin));
        user1.doDaiExit(address(usdaiJoin), address(user1), 0.05 ether);
        assertEq(usdai.balanceOf(address(user1)), 0.05 ether);
    }
    ...
    function testEnd() public {
        ...
        vat.frob("ETH", address(this), address(this), address(this), 2 ether, 400 ether); // Maximun USDAI generated
        pipETH.poke(bytes32(uint(300 * 10 ** 18 - 1))); // Decrease price in 1 wei
        spotter.poke("ETH");
        ...
        user1.doTend(address(ethFlip), batchId, 1 ether, rad(100 ether));
        user2.doTend(address(ethFlip), batchId, 1 ether, rad(140 ether));
        assertEq(vat.usdai(address(user2)), rad(860 ether));
        ...
        assertEq(vat.usdai(address(user2)), rad(1000 ether));
        (ink, art) = vat.urns("ETH", address(this));
        ...
        (ink, art) = vat.urns("ETH", address(this));
        uint remainInkVal = 2 ether - 400 * end.tag("ETH") / 10 ** 9; // 2 ETH (deposited) - 400 USDAI debt * ETH cage price
        ...
        vow.heal(vat.usdai(address(vow)));
    ...
    function testFlopEndWithBid() public {
        ...
        uint batchId = _flop();
        user1.doDent(address(flop), batchId, 0.6 ether, rad(20 ether));
        assertEq(vat.usdai(address(user1)), rad(800 ether));
        this.cage(address(end));
        flop.yank(batchId);
        assertEq(vat.usdai(address(user1)), rad(820 ether));
    }
    ...
    function testDsr() public {
        ...
        ethJoin.join(address(this), 0.5 ether);
        vat.frob("ETH", address(this), address(this), address(this), 0.1 ether, 10 ether);
        assertEq(vat.usdai(address(this)), mul(10 ether, RAY));
        ...
        pot.drip();
        pot.exit(10 ether);
        assertEq(vat.usdai(address(this)), mul(10.5 ether, RAY));
    }
    ...
    function testAuth() public {
        ...
        // vat
        assertEq(vat.wards(address(ussDeploy)), 1);
        ...
        // ussDeploy
        assertEq(address(ussDeploy.authority()), address(0));
        assertEq(ussDeploy.owner(), address(this));

        ussDeploy.releaseAuth();
        ussDeploy.releaseAuthFlip("ETH");
        ussDeploy.releaseAuthFlip("COL");
        ussDeploy.releaseAuthClip("COL2");
        assertEq(vat.wards(address(ussDeploy)), 0);
        assertEq(cat.wards(address(ussDeploy)), 0);
        assertEq(dog.wards(address(ussDeploy)), 0);
        assertEq(vow.wards(address(ussDeploy)), 0);
        assertEq(jug.wards(address(ussDeploy)), 0);
        assertEq(pot.wards(address(ussDeploy)), 0);
        assertEq(usdai.wards(address(ussDeploy)), 0);
        assertEq(spotter.wards(address(ussDeploy)), 0);
        assertEq(flap.wards(address(ussDeploy)), 0);
        assertEq(flop.wards(address(ussDeploy)), 0);
        assertEq(cure.wards(address(ussDeploy)), 0);
        assertEq(end.wards(address(ussDeploy)), 0);
        assertEq(ethFlip.wards(address(ussDeploy)), 0);
        assertEq(colFlip.wards(address(ussDeploy)), 0);
        assertEq(col2Clip.wards(address(ussDeploy)), 0);
    }
}
